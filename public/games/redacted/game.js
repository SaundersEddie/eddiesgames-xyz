/* MAPHIA Prototype (click-trace word game)
   - 5x5 grid
   - select adjacent tiles (no reuse)
   - submit guesses (dictionary-backed, words.json)
   - hidden secret word (6–8 letters) traceable in grid
   - feedback: Heat + Order Hit
*/
...
(() => {
  ...
  async function loadWordList() {
    // Must exist at: public/games/maphia/words.json
    // Supported formats:
    // 1) ["word1","word2",...]
    // 2) [["word", freq], ["word", freq], ...]
  ...
  const LETTER_BAG = [
    ...
    ..."nnnnnn", ..."rrrrrr", ..."tttttt", ..."llll", ..."ssss",
    ..."uuuu", ..."dddd", ..."ggg", ..."bb", ..."cc", ..."mm",
    ..."pp", ..."ff", ..."hh", ..."vv", ..."ww", ..."yy",
    ..."k", ..."j", ..."x", ..."q", ..."z"
  ];
  ...
  function neighbors(idx) {
    ...
    for (let dr = -1; dr <= 1; dr++) {
      for (let dc = -1; dc <= 1; dc++) {
        ...
        if (rr >= 0 && rr < SIZE && cc >= 0 && cc < SIZE) out.push(rcToIdx(rr, cc));
      }
    }
    return out;
  }
  ...
  function canTraceWord(grid, word) {
    ...
    const starts = [];
    for (let i = 0; i < grid.length; i++) if (grid[i] === word[0]) starts.push(i);
  ...
  let guesses = [];
  let selected = []; // indices in selection order
  let locked = false;
  ...
  function pickSecretWord(rngFn, grid) {
    ...
    const candidates = SECRET_CANDIDATES.slice(); // copy so shuffle is safe
  ...
  function startCase({ daily = true } = {}) {
    ...
    const seed = daily ? seedFromDate(new Date()) : (Math.floor(Math.random() * 1e9) >>> 0);
    rng = mulberry32(seed);
    seedLabelEl.textContent = daily ? `Seed: ${seed} (Daily)` : `Seed: ${seed} (Random)`;
  ...
  function submitCurrent() {
    ...
    if (w === secret) {
      locked = true;
      openModal("Case Closed.", `You nailed it: ${secret.toUpperCase()} • Win in ${guesses.length}/${MAX_TURNS}.`);
      return;
    }
    ...
    if (turnsLeft <= 0) {
      locked = true;
      openModal("Case Went Cold.", `Out of turns. The word was ${secret.toUpperCase()}.`);
      return;
    }
  ...
  (async () => {
    try {
      ...
      WORDS = ["trace","cipher","signal","secret","shadow","letter","market","corner","silent"];
      DICT = new Set(WORDS);
      SECRET_CANDIDATES = WORDS.filter(w => w.length >= 6 && w.length <= 8);
      startCase({ daily: true });
    }
